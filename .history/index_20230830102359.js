...
app.post("/", async (req, res) => {
  ...
  if (text === "") {
    ...
    response = `ENN you will receive an sms with a link to a document about it shortly \n`;
  } else if (text === "3") {
    function sendSms() {
      console.log("wwwww");
      const options = {
    ...
    try {
      const ussdResponse = await axios.post(
        "https://hiremate-hnjv.onrender.com/stkpush", // Replace with your USSD server URL
        {
